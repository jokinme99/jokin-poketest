...
class PokemonManager {
    ...
    func fetchList( _ completion:  @escaping  (PokemonListData?, Error?) -> Void) {
        let limit = 1126
        let pokemonListURL = "https://pokeapi.co/api/v2/pokemon/?limit=\(limit)"
                ...
                case .success(let data):
                    if self.saveDataOffline ?? true {
                        if DDBBManager.shared.get(PokemonListData.self).isEmpty ||
                            DDBBManager.shared.get(PokemonData.self).isEmpty ||
                            DDBBManager.shared.get(PokemonFilterListData.self).isEmpty {
...
extension PokemonManager {
    func addOfflineData(_ pokemons: PokemonListData) {
        ...
        let pokemonTypes =  [
            TypeName.normal.rawValue, TypeName.fighting.rawValue, TypeName.flying.rawValue, TypeName.poison.rawValue,
            TypeName.ground.rawValue, TypeName.rock.rawValue, TypeName.bug.rawValue, TypeName.ghost.rawValue,
            TypeName.steel.rawValue, TypeName.fire.rawValue, TypeName.water.rawValue, TypeName.grass.rawValue,
            TypeName.electric.rawValue, TypeName.psychic.rawValue, TypeName.ice.rawValue, TypeName.dragon.rawValue,
            TypeName.dark.rawValue, TypeName.fairy.rawValue, TypeName.unknown.rawValue, TypeName.shadow.rawValue]
    ...
    func isSavedPokemonFilterData(_ pokemonList: PokemonFilterListData) ->
        (isSaved: Bool, saved: PokemonFilterListData?) {
        let filter = "name == '\(pokemonList.name!)'"
